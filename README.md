...
| | 内容 |
|---|---|
| 画像 | **42枚**（`images/` に入っています） |
| 投稿文 | **42本分**、`posts.csv` に作成済み（たたき台） |
| 投稿予定 | **2026年8月25日 〜 10月5日**（1日1本） |
| 投稿時刻 | 日本時間 **20:00** |
| 投稿先 | Threads **@hiromi.lifeplan**（同じアカウントセンターに削除予定の shu1_omatomepan があるので取り違え注意） |

漫画が連作（つながった物語）なので、**番号順に投稿する並び**にしてあります。

1. `1.jpg` 〜 `33.jpg` … 資産家との出会いから始まる長編（8/25〜9/26）
2. `page_01.jpg` 〜 `page_09.jpg` … 退職後のお金の話（9/27〜10/5）
...
```
2026-10-06,images/34.jpg,Xに出す文,Threadsに出す文,
```

これで、2026年10月6日の20時に自動で投稿されます。

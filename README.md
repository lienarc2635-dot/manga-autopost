...
| | 内容 |
|---|---|
| 画像 | **120枚**（`images/` に入っています） |
| 投稿文 | **120本分**、`posts.csv` に作成済み |
| 投稿予定 | **2026年8月23日 〜 9月17日**（1日3枚のツリー）<br>**2026年9月22日 〜 11月2日**（1日1ページ） |
| 投稿時刻 | 日本時間 **朝6:00** |
...
3. `page-01` 〜 `page-42`（42日分）… 労働脳から資産家脳へ。1枚が1ページの漫画なので、**1日1枚**の投稿です（9/22〜11/2）。
   元の画像は `C:\AI\manga\manga_page_01.png` 〜 `42.png`。

1日に何枚出すかは `posts.csv` で同じ日付の行が何行あるかで決まります。
1行ならThreadsは普通の画像投稿、Instagramも1枚の通常投稿になります（プログラムの変更は不要）。
...
Threads と Instagram のトークンは、どちらも **60日で切れます**。
`GH_PAT` を登録しておくと、毎週月曜の朝に両方とも自動で延長されるので、手作業の更新が不要になります
（`refresh_threads_token.py` と `refresh_instagram_token.py`）。

Instagramのトークンは「切れる前」でないと延長できません。
一度切れてしまったら、Meta for Developers で取り直して `INSTAGRAM_ACCESS_TOKEN` を貼り直してください。
...
朝の漫画とは別に、**毎晩19時に文章だけの投稿**をThreadsへ自動投稿します。

| | 朝6時 | 夜19時 |
|---|---|---|
| 内容 | 漫画1ページ（第1部は3枚のツリー） | 文章だけ1本（第1部は2〜3本のツリー） |
| 投稿先 | Threads のみ | Threads のみ |
...
`.github/workflows/evening-post.yml` の `- cron: '5 10 * * *'` を書き換えます。
UTC 10:05 が日本時間19:05です。21時にするなら `'0 12 * * *'` です。GitHubの実行は数時間遅れることがあるので、出したい時刻の2〜3時間前に設定しておくと狙った時間帯に出ます。
